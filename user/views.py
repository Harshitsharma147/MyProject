...
def morder(request):
    ...
    if msg is not None:
        ...
        cartitems =cart.objects.filter(userid=user).count()
        request.session['cartitems'] = cartitems
        return HttpResponse("<script>alert('your order placed successfully..');location.href='/user/orderslist/'</script>")
        x=cursor.fetchall()
        print(x)
...
def  orderslist(request):
    ...
    pdata=myorders.objects.filter(userid=user,status="Pending").order_by('-id')
    adata=myorders.objects.filter(userid=user,status="Accepted").order_by('-id')
    ddata=myorders.objects.filter(userid=user,status="Delevered").order_by('-id')
    if oid is not None:
        myorders.objects.filter(id=oid).delete()
        return HttpResponse("<script>alert('your order hase been canceled...');location.href='/user/orderslist/'</script>")
